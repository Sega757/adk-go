...
## 2026-09-18 - Drag-and-Drop Image Attachments & Window Navigation Safeguards in Web Chat
**Learning:** In streaming web chat applications, dropping image files into the browser window without drag-and-drop handlers causes the browser to navigate away from the app to display the raw file URL, interrupting active WebSocket sessions and losing chat context. Preventing default window dragover/drop events while attaching a dragover focus state (`.drag-over`) to input containers allows users to effortlessly drop image files into chat while preserving session state.
**Action:** Always override default `dragover` and `drop` window events in web chat applications, highlight input target containers on active drag hover, and process dropped image files through existing attachment pipelines.
