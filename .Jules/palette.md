...
## 2026-09-07 - Web Log Console Empty State Guidance
**Learning:** In real-time streaming web applications with side-by-side event consoles, displaying an empty container before events arrive or after the console is cleared creates visual confusion and leaves users wondering if the application is broken or hung. Providing an explicit empty state placeholder with subtle italic guidance ("No events recorded yet...") inside `role="log"` containers, toggled dynamically when entries are added or cleared, provides immediate visual feedback and improves onboarding clarity.
**Action:** Always include a styled empty state element inside log/console containers, hide it on first entry addition, and restore it when clearing the log.
