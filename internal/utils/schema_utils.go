...
func matchType(value any, schema *genai.Schema, isInput bool) (bool, error) {
	...
	// Fast-path standard uppercase and lowercase schema types to avoid heap allocations
	// from strings.ToUpper in the common case.
	switch schema.Type {
	...
	case genai.TypeObject, "object":
		...
		return err == nil, err
	default:
		// Fall back to case-insensitive conversion for non-standard mixed-case types.
...
func ValidateMapOnSchema(args map[string]any, schema *genai.Schema, isInput bool) error {
	...
	// Go map lookups on nil maps safely return (nil, false) without allocating a new map.
