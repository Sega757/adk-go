...
//
// Performance optimization by Bolt: For primitive JSON-safe types, if we can validate
// the value directly (or avoid it if resolvedSchema is nil) and type-assert it directly to 'To',
// we bypass expensive JSON marshal and unmarshal round-trips.
...
//
// Performance optimization by Bolt: If the input value is already a primitive JSON-safe
// type, we can validate it directly, bypassing expensive JSON marshal and unmarshal cycles.
