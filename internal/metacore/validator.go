...
var (
	...
	ErrNilPacket            = errors.New("decision packet cannot be nil")
	ErrInvalidConfidence    = errors.New("confidence must be between 0.0 and 1.0 and not NaN")
	ErrInvalidVulnerability = errors.New("vulnerability score must be between 0.0 and 1.0 and not NaN")
)
...
func (v *Validator) ValidatePipeline(packet *DecisionPacket, empathy *EmpathyOutput) (*KillSwitchOutput, error) {
	// 0. Validate inputs to prevent nil pointer dereferences and NaN/out-of-bounds bypass attacks
	if packet == nil {
